...
- `relevance` is now assigned on **every** path through the daily classification loop. It used to be set on two of five: `category_hint` hard-coded it to `中`, and the AI-exception and short-content paths left it unset for the writer's `fm.get('relevance', '中')` to catch. Measured over the 2201 stored articles, 2199 of them carried that default - which meant `generate_ai_report.py`'s `if topic != FOCUS_TOPIC and relevance != '高': continue` had never filtered on anything but topic.
