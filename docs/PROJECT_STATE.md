...
| Official-account daily | Filter configured sources, preserve source categories, backfill incomplete yesterday output before an unqualified today run, fetch articles, classify them, create summaries, generate a local HTML reader, and synchronize reader favorites into local files. Topic (6-way) and relevance (3-level) are decided by TypeSafe's Jev when `typesafeApiKey` is configured, and by the previous LLM-parse path otherwise; `--classifier llm` forces the latter. | `daily`, `daily favorites`, `daily-stats`, `daily-server` |
