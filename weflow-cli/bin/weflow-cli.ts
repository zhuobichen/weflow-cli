...
async function showInteractiveMenu() {
  ...
  const runPython = async (scriptName: string, label: string, extraArgs = '') => {
    const { execFile } = await import('child_process')
    const { promisify } = await import('util')
    const execFileAsync = promisify(execFile)
    const { fileURLToPath } = await import('url')
    ...
    const args = extraArgs ? [script, ...extraArgs.split(' ')] : [script]
  ...
  switch (action) {
    ...
    case 'extract-todos': {
      ...
      if (sessions.length === 0) { console.log(chalk.gray('无会话')); break }
      const { talker } = await inquirer.prompt([{
        type: 'select' as any, name: 'talker', message: '选择联系人（提取待办事项）',
        choices: sessions.map((s, i) => ({
          name: `${String(i + 1).padStart(3)}. ${s.displayName || s.username}`,
          value: s.username,
        })),
      ...
      const { days } = await inquirer.prompt([{
        type: 'select' as any, name: 'days', message: '扫描最近多少天',
        choices: [
          { name: '7 天', value: '7' },
          { name: '14 天', value: '14' },
          { name: '30 天（默认）', value: '30' },
          { name: '90 天', value: '90' },
        ],
        loop: false,
      }])
      await runPython('extract_todos.py', '提取待办事项', `--talker "${talker}" --days ${days}`)
      break
    }
    case 'biz-daily':
