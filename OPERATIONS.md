...
```powershell
weflow-cli draft 老王 --dry-run        # 预览：会发多少字符给哪两个模型（不出境、不花钱）
weflow-cli draft 老王 --yes            # 真跑：给 3 条候选，按"最适合当下一条"排序
weflow-cli draft 老王 --count 5 --yes  # 最多 5 条
```

**它只产出文字，不会替你发送**——没有回填输入框、没有模拟按键、没有抢焦点。
回不回、怎么回由你决定（本机面板里助手的每条回答下面有复制按钮，微信里那侧没有）。

跑的时候先**判断**再**起草**：判断那一步问七件事（对方想干什么、要什么、该用哪种动作、
该不该给实质内容、风险有多高、涉不涉及钱、有没有没兑现的承诺），结论会写进起草的提示词里，
并把你**自己以前说话的样子**当作语气样本。判断和排序各一次决策调用，起草一次生成调用，
所以是三次。

- **涉钱、或风险 ≥ 7 时不给草稿**，只说明风险在哪、该先确认什么。这不是失败，是这条
  最该给的回答。风险的 0-9 是**拍的阈值**，用来解释，不单独用来决定。
- 承诺未兑现**不会**被拒（"我明天一定发你"正是该帮写的时候），改在提示词里约束不让它
  替你许新的承诺；想让它也硬拒，加 `--gate-commitment`。
- **没有金标准校准过**：判断准不准只有"和你自己读一遍对比"，不是校准过的结论。
- 判断用的是英文判据，而 `awaiting` 那两问是中文——同一个模型上的两套口径，**没做过 A/B**。
- 群聊不适用。

微信里让助手起草（工具名 `draft_reply`）走的是同一条实现，但**先把正文遮罩再交出去**，
且隐私模式为 `strict` 时直接拒绝：起草必须把正文发给远端模型，遮成"内容 N 字"之后再起草
只会得到一段看起来正常的空话。
...
**球怎么用**：**点一下**在它旁边展开对话气泡（点球本身或气泡标题栏上的"收起"都能收回去，
球也可以再点一下——它是个开关）。**球全程不动**，它不消失也不淡出，气泡从它另一侧长出来。
气泡默认开在球的**左边**（球默认在屏幕右下角，所以往左上长不遮屏）；球被拖到屏幕左缘时，
左边不够地方，气泡会自己翻到右边；上方不够就改成与球顶对齐。**拖动球**换位置（4 像素内算点击、
超过才算拖），位置记在 `~/.weflow-cli/panel_position.json`，下次打开还在那儿；显示器换了或
球被拖到屏幕外时会回到右下角。展开后气泡这一整块**不置顶**——去点别的窗口时它会到后面去，
从任务栏点回来即可。
