...
- **判断那一步调不通时（Jev 免费期已过 / key 失效 / 网络断）会自动降级**：只用 DeepSeek 给出候选。
  但**那道闸门就没有了**——它本来就是靠判断结果才存在的。所以降级时输出里会**先写一行**
  「这次没有判断…闸门没生效」，候选照给。看到这行时请自己看一眼再决定发不发。
  （三个 key 里现在**只有 `deepseekApiKey` 是必须的**。）
...
在面板窗口里**点右键**，弹出一个菜单：名单里那几个人，加上最后一项「关闭悬浮球」。
点一个人，助手就给那个人起草候选。名单是一个配置键：

```powershell
weflow-cli config set quickReplyContacts "咸鱼梦想家,彪弟,平凡的世界"   # 逗号分隔；改完**重启助手**
```

- 名单里写**联系人名**（就是你平时叫他的那个，和 `weflow-cli messages` 一样解析）。**改完要重启助手**（`assistant stop` 再 `assistant start`）——配置是守护进程启动时读进内存的，面板最多再等 30 秒（它每 30 秒轮询一次状态）。
  重名会得到"匹配多个会话"这种可读的失败——**不猜**。
- 候选回在**对话气泡里**（就是平时问答的那个位置，最近 30 条对话是它的依据）。
- 菜单底部写着代价：**起草会把这段对话发给两个云端模型**（只产出文本，不会替你发送）。
  点一下就是一次出境，所以那句话必须在菜单里看得见，而不是点完才知道。
- 弹的是**原生菜单**，画在窗口外面：所以球形态下窗口**不会变大、球也不会动**（上一版是「先把窗口展开再弹」，用户看到的就是「点右键把第二大脑窗口弹出来了」——那不是快速功能该有的样子）。
- 没配名单时菜单不弹空框，直接写着 `config set quickReplyContacts` 该跑的那条命令。
- 菜单**最后一项**是「**关闭悬浮球（托盘图标能再打开）**」：点它只是把球**收起来**（`win.hide()`），
  助手照常在后台跑。叫回来有三条路：点托盘图标、托盘菜单的「显示 / 收起」、`Ctrl+Shift+W`。
  之所以在标签里写出"托盘图标能再打开"：球是 `setSkipTaskbar(true)`，藏起来之后任务栏里没有它，
  不写清楚就像"点一下它永远消失了"。真要退出用**托盘菜单**那两项（「退出面板」/「退出并停止助手」）——
  "收起来"和"退出"是两件事，不放在同一个菜单里。
  这一项在**名单为空时也在**，否则没配过名单的人连球都收不起来。
- 浏览器降级那条路**不接管右键**（Edge/Chrome 原生菜单里的「复制」要留给用户），那边也就没有快速回复——要起草就在对话里直接问一句。
...
建索引时要先决定**收多长时间的数据**，这决定了"知识库记得住多少"：

```powershell
weflow-cli search-index --dry-run            # 先看窗口与代价（不读库、不联网、不写文件）
weflow-cli search-index --yes                # 默认：聊天最近 90 天 + 日报最近 30 个日期目录
weflow-cli search-index --days 3650 --article-days 365 --yes   # 要"整段历史"就显式给大值
```

- 默认值与从前**完全一致**（90 / 30）——加参数不是改代价，只是把原来写死、改不了的数字
  变成改得了的。想建"整份聊天记录"的知识库，就显式给一个大窗口。
- **天数越多，发给向量服务的文本越多**，这是出境成本，所以确认语里会写明这次用的窗口。
- 上限 36500 天（≈100 年）；`0` 会被拒——0 天等于建出一个**空**索引，而它看起来像"建好了"。
...
四条命令，各管一段（**故意不做成一条大命令**：中间两步都要出境，合成一条会把"这一步要发什么出去"
藏起来）：
...
知识库有两条来源：**公众号文章**（`output/biz-daily` → `wiki compile`）和**你自己的微信对话**。
后者分两步走，第二步是现成的：
...
- **文章线要先用一道提炼**（2026-09-26 实测后补上）：
  ```powershell
  weflow-cli article-notes --dry-run              # 先看代价（只读本地）
  weflow-cli article-notes --limit 30 --yes       # 每篇一次调用，**只问概念**（摘要照抄原笔记）
  weflow-cli wiki compile --source ./output/article-notes --yes
  ```
  实测：30 篇 → 110 个概念 → 生成 10 个概念页（`output/wechat-vault/Wiki/Concepts/`，
  另加一张索引 `Wiki/00-Overview.md`）。生成的概念是「标签配料表一致性」「椰子水全覆盖风险排查」
  这种具体的，不是主题词。
- **为什么必须有这道提炼**（历史）：：`wiki compile` 要的是「概念形状」的链接
  （`[[概念]] — 关于它说了什么`），而 Vault 里那 1633 篇文章笔记里**一条都没有**——正文里的
  `[[…]]` 只有两类：与自己主题同名的那条（`> - **主题**: [[AI]]`），以及 `## 🔗 关联网络` 里的路径互链。
  这两类**都不是概念**（拿它们当概念跑过：引用榜首是「新闻 752 / 政治 388 / 学术 341」加两条 `.md` 路径），
  所以聚合器现在会把它们滤掉，结果是文章线暂时聚不出东西。要有产出，得再加一道「从文章提炼概念」的
  步骤（对话线那道就是 `chat_notes`）。
- 太薄的会话（少于 3 条消息或 60 字）**不产卡**，并在输出里列出来——几句话的对话写成"知识卡"
  只会是"只说了两句话"，还白占一次调用、往概念页里灌一条没有信息量的来源。
- 卡是**滚动覆盖**的：同一个会话永远只对应一个文件，重跑就重写（记录 `updated` 与窗口），
...
```powershell
weflow-cli wiki lint            # 断链 / 孤儿 / 空页 / 同名
weflow-cli wiki lint --json     # 机器可读
```

- **断链**：`## 来源` 里指向的卡片没了（卡片删了或改名）——这是**真坏了**；
- **还没建页的相关概念**：`## 相关概念` 里模型提出、但当前 `--limit` 还没轮到建页的——
  这是**扩张候选，不是错误**（实测首次跑：38 个候选、0 个断链）。两类分开报，是为了让你
  还能信任这份报告；混在一起报成"40 条死链"，下次就没人看了；
- **孤儿**：连卡片都没提到过它（卡片链接也算入链，否则每张页都会被误报）；
- **空页**：没有定义或要点——多半是那次模型返回不完整，重跑 `wiki compile` 就好。

**两条线都是默认增量**：`article-notes` 只处理还没卡的（原笔记改了才重做，`--refresh` 强制重做），
`chat-notes` 的卡是滚动快照（本来就该覆盖）。所以"补新的"不会把旧的再问一遍。
...
**token 过期这件事现在会说出来。** 服务端用 `errcode: -14` 表示这个 token 已失效，而**重试不可能成功**；
在这之前本仓查的是别的码，于是它把"过期"当成普通错误、每 5 秒重试一次、永远重试下去——用户那边
看不出任何异常（`channelActive` 当时只表示"配了 token"）。现在：

- 日志里会出现**一行明确的**话（只喊一次，不刷屏）：`微信通道的 token 已失效……重新登录：weflow-cli login-wechat`；
- 面板与本机状态里的 `channelActive` 会**如实变 false**（也就是显示"仅本机入口"），而不是继续报"微信 + 本机"；
- 处理办法就是去重新登录，**然后重启助手**（运行中的守护进程手里还是那个旧 token）：
  `weflow-cli login-wechat` → `weflow-cli assistant stop` → `assistant start`。
  在此之前**本机面板那条入口照常可用**，助手不是整个不能用。
- 其余失败（网络抖动等）仍然重试，但改成 1s→30s **指数退避**：原来固定 5 秒一次，长时间断网会把
  日志刷成噪音，而噪音等于没说。通道恢复后计数归零。
